...
def _to_do_in_parallel(train, test,estimator=None, R={}, y=None, _t=0, Thetas={}):
    Rtr = dict()
    Rts = dict()
    for r in R:
        if r[0] == _t:
...
def _shuffle_split(estimator, R, y, _t,  Thetas=dict(), n_splits=5,
    ...
    for train, test in ss.split(np.arange(0,n), y):
    	scores = _to_do_in_parallel(train,test, estimator=estimator,
				    R=R, Thetas=Thetas, y=y, _t=_t)
...
def _create_theta(T, types, dimensions):
    n1 = int(np.sum(list(dimensions.values())))
...
def _create_G(types, dimensions, ranks, _type=None, init=None, random_state=None):
    n1 = int(np.sum(list(dimensions.values())))
    n2 = int(np.sum(list(ranks.values())))
    ...
    for t in types:
        r = dimensions[t]
        c = ranks[t]
        if t == _type and type(init) != str:
            G[shift_row:shift_row+r, shift_col:shift_col+c] = init
        else:
...
def _update_G(G, R, S, T):
    ...
    num = RGS_p + G.dot(SGGS_n) + t_n.dot(G) 
    denom = RGS_n + G.dot(SGGS_p) + t_p.dot(G) + 1e-10
    where_zeros = np.where(denom==0)
...
def _update_S(G, R):
    GTG_inv = np.linalg.pinv(G.T.dot(G), rcond=1e-6)
    GRG = G.T.dot(R).dot(G)
...
def _tri_smtf(R, T, dimensions, ranks, _type, init=None, max_iter=100, stopping=None,
    ...
    G = _create_G(types, dimensions, ranks, _type=_type, init=init,
...
class tri_SMTF(BaseEstimator, ClusterMixin, TransformerMixin):
    ...
    def fit(self, Rs, y, Thetas=dict()):
       ...
       # y = y.ravel().astype(int)
        ...
        #self.lr = LogisticRegression(C=self._lambda)
    ...
    def predict(self, Rs,  _type, Thetas=dict(), init=None):
    ...
    def transform(self, Rs, Thetas, _type, init=None):
    ...
    def score(self, Rs=None, y=None, _type=None, Thetas=dict()):
        ...
        check_is_fitted(self, 'labels_')
        if Rs is None:
            labels = self.prediction = self.lr.predict(
                self.G_[self._type])
            Rs = self.Rs
        else:
        ...
        return matthews_corrcoef(y, labels)
        # total_relations = 0
        # silhouette = 0
        # print(labels)
        # for r in Rs:
