...
#
#     gamma_i = [1e-10 for X_i in X]#[2/np.linalg.norm(X_i.T.dot(X_i)) for X_i
#               in X]
#     obj = float("inf")
#     checks = []
...
def _fit_ensemble_en(X, y, alphas=0.01, l1_ratios=0.5, max_iter=200,
    ...
    gamma_i = 1e-10  # [2/np.linalg.norm(X_i.T.dot(X_i)) for X_i in X]
    obj = float("inf")
    checks = []
    for _iter in range(max_iter):
        w_old = w.copy()
        ...
        for i in range(len(W)):
            W[i] = soft_thresholding(W[i] - gamma_i*grad_W[i],
                                     gamma_i*taus[i+1])
...
class EnsembleClassifier(ClassifierMixin):
    ...
    estimator_params: for each estimator
                      -in the case of elastic net:
                        a list of tuples of length two with an alpha
                        (regularization coefficient) and l1_ratio. If the same
                        parameters have to be used for all the estimators just
                        one tuple with the parameters.
    """
...
class EnsembleClassifierCV(ClassifierMixin):
    """Base RandomCV.

    """

    def __init__(self, model, params, fit_params=None, cv=3):
